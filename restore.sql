...
create materialized view active_users_fall_2018_fall_2019 as (
  select count(distinct annotation.userid) as count, cast(annotation.created as date) as date_created
  from annotation 
  where (  annotation.created > date('2018-08-15') and annotation.created < date('2018-12-30') )
  or (annotation.created > date('2019-08-15') and annotation.created < date('2019-12-30'))
  group by cast(annotation.created as date)
  order by cast(annotation.created as date) 
) with data;

grant select on active_users_fall_2018_fall_2019 to metabase;
